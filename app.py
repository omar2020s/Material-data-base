...
def split_search_terms(search_text: str):
    """
    Allows searching for several material types/words at the same time.
    Examples:
    - bonding - curing
    - bonding, curing
    - bonding / curing
...
def index():
    ...
    if old_type and old_type != "All Types" and old_type not in selected_types:
        selected_types.append(old_type)

    material_types = get_material_types()
    stats = get_dashboard_stats()

    # The table appears after choosing at least one type OR typing a search term.
    # This prevents showing the full database by default, but allows fast multi-type search.
    materials = []
    table_ready = bool(selected_types or search_terms)

    if table_ready:
        query = Material.query

        if selected_types:
            query = query.filter(Material.material_type.in_(selected_types))
