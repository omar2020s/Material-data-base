...
def index():
    ...
    # Materials table appears only after selecting a material type.
    materials = []
    table_ready = bool(type_filter)

    if table_ready:
        query = Material.query.filter(Material.material_type == type_filter)
...
def save_material():
    ...
    return redirect(url_for("index", type=material_type))
...
def calculate():
    try:
        ...
        return redirect(url_for("index", type=request.form.get("type_filter", "")))
...
def edit_material(material_id):
    ...
    materials = (
        Material.query.filter(Material.material_type == material.material_type)
        .order_by(Material.name.asc())
        .all()
    )
    ...
    return render_template(
        ...
        search="",
        type_filter=material.material_type or "",
        table_ready=True,
        stats=stats,
    )
