...
# ================= DATABASE CONFIG =================
# Render PostgreSQL gives DATABASE_URL like postgresql://...
database_url = os.environ.get("DATABASE_URL", "sqlite:///local_materials.db")

# Use psycopg v3 driver on Render PostgreSQL
...
def ensure_database_schema():
    ...
    This avoids errors like: column material.rate does not exist.
    """
    db.create_all()
...
def normalize_datasheet(source: str) -> str:
    ...
    if source.lower().startswith(("http://", "https://")):
        return source
    # On Render/local web app, local PC paths will not work for users online.
    # So if user enters a plain domain, convert it to https link.
...
def calculate():
    ...
    selected_materials = Material.query.filter(Material.id.in_(selected_ids)).order_by(Material.name.asc()).all()
