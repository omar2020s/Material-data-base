...
def ensure_database_schema():
    """
    Creates tables and safely adds missing columns when an old PostgreSQL table exists.
    This keeps existing material data and does not delete anything.
    """
    db.create_all()
...
def get_selected_types_from_request():
    """
    Correct multi-type reading:
    URL will be like: /?types=Curing&types=Waterproof%20Membrane
    We use request.args.getlist("types") not request.args.get("types").
    """
    selected = request.args.getlist("types")
    return [x.strip() for x in selected if x and x.strip()]
...
def get_dashboard_stats():
    ...
    latest_material = Material.query.order_by(Material.created_at.desc().nullslast()).first()
    return {
        "total_materials": total_materials,
        "total_types": total_types,
        "avg_price": avg_price,
        "latest_material": latest_material.name if latest_material else "-",
    }
...
def index():
    ...
    selected_types = get_selected_types_from_request()

    material_types = get_material_types()
    stats = get_dashboard_stats()

    # IMPORTANT:
    # Do not show all materials by default.
    # Show materials only when one or more material types are selected.
    materials = []
    show_results = bool(selected_types)
        ...
        if search:
            ...
            query = query.filter(
                or_(
                    ...
                    Material.datasheet.ilike(like_text),
                )
            )
...
def save_material():
    ...
    # Return to selected type if material type exists
    if material_type:
...
def edit_material(material_id):
    ...
    selected_types = get_selected_types_from_request()
    if not selected_types and material.material_type:
        selected_types = [material.material_type]

    materials = []
    if selected_types:
        materials = (
            Material.query
            .filter(Material.material_type.in_(selected_types))
...
def calculate():
    ...
    if not selected_ids:
        ...
        # Keep selected type filters after failed calculation
